---
name: framework
description: Guide for the Tesseron TypeScript protocol and SDK — apps, actions, resources, ActionContext, transports, React hooks, session lifecycle, MCP gateway configuration. Use when code imports from `@tesseron/core`, `@tesseron/web`, `@tesseron/server`, `@tesseron/react`, or `@tesseron/mcp`, calls `tesseron.action(...)`, `tesseron.resource(...)`, `tesseron.connect(...)`, `useTesseronAction`, `useTesseronResource`, or `useTesseronConnection`, or the user asks about exposing typed web-app actions to Claude or other MCP agents over WebSocket.
---

...
FetchContent_Declare(
  tesseron
  GIT_REPOSITORY https://github.com/Eigenwise/tesseron.git
  GIT_TAG main
...
`SOURCE_SUBDIR` is the part that matters: the repository is a monorepo, and the C++ project is the `sdks/cpp` subtree. Nothing under that subtree refers to a path above it, so the same declaration keeps working when the SDK moves to its own repository.
...
From the repository root. Never change directory into `sdks/cpp` first; `-S` and `-B` are what keep the build runnable from anywhere.
...
```bash
cmake -S sdks/cpp -B sdks/cpp/build -DCMAKE_INSTALL_PREFIX=/your/prefix
cmake --build sdks/cpp/build --target install
```

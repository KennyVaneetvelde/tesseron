...
All six examples live in [`sdks/typescript/examples/`](https://github.com/eigenwise/tesseron/tree/main/sdks/typescript/examples). Each is complete, runnable, and intentionally simple so the Tesseron-specific code is easy to read. Client-side examples share a **todo** domain; server-side examples share a sampling-heavy **prompt-library** domain.

...
(`@tesseron/mcp` 2.10.4), the docs server at `docs-mcp/` (`@tesseron/docs-mcp` 2.10.5), the
...
there), `tesseron-rust` (crate `tesseron` 0.2.0), `tesseron-python` (PyPI `tesseron` 0.2.0), and
`tesseron-cpp` (v0.2.0, CMake `FetchContent`, target `tesseron::tesseron`). The gateway depends on

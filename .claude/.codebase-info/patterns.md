...
| Issue | Guard | Where |
|---|---|---|
| tesseron#88 | `connectVersion` counter, sync supersede-close, await prior chain, bail if superseded | `tesseron-typescript/core/src/client.ts:273-342` |
| tesseron#88 | URL-form connect de-duplication via `inFlightUrlConnect` | `tesseron-typescript/web/src/index.ts:97` |
...
The Vite plugin's `Session` (`tesseron-typescript/vite/src/index.ts:85`) is deliberately decoupled from the
...
`tesseron-typescript/web/src/reactive-core.ts` exists because react, svelte, and vue had drifted byte-for-byte
...
reaper (`tesseron-typescript/core/src/client.ts:697`, reaper at `:776`) so a handler that ignores `ctx.signal`
...
- Constant-time compare for the resume token (`gateway.ts:1451`) and host-side bind codes
  (`tesseron-typescript/server/src/transport.ts:174`). The legacy claim-code lookup is **not** timing-safe
